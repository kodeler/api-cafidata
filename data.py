...
class Data:
    def __init__(self, df, target_parameter, df_variables, nan_method="drop"):
        # indica los parametros que se van a usar para el proyecto en cuestion (target_parameter, df_variables) y el metodo de llenado de NaN
        ...
        self.project_source = "projectSource"
        # funciona para el proyecto de produccion

        # keep as original data (no in place changes, just in case)
        self.df = df # dataframe original 
        self.processed = False # procesa el dataframe original y lo guarda en self.df_processed (no in place changes, just in case)

        if target_parameter is not None: # si el target_parameter no es None, entonces se va a usar para el proyecto en cuestion
            self.processed = True # se ha procesado el dataframe original

            self.target_str = target_parameter # el target_parameter es el parametro que se va a usar para el proyecto en cuestion
            self.df_vars_str = df_variables # los df_variables son los parametros que se van a usar para el proyecto en cuestion

            # process dataframe
            self.nan_method = nan_method # el metodo de llenado de NaN
            self.processed_df = self.nan_handler(self.df, self.nan_method) # procesa el dataframe original y lo guarda en self.df_processed

            self.target = self.processed_df[target_parameter] # el target_parameter es el parametro que se va a usar para el proyecto en cuestion
    ...
    def make_onehot_df(df):
        ...
        for feature in list(categorical_features):
            ...
            for uval in unique_vals:
                ...
                # asume que no hay columna con función de nombre + str(uval) 
